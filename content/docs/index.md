---
title: Private Chat App Documentation
description: Welcome to the Private Chat App documentation. Learn how to use and understand the codebase.
order: 2
---

# Welcome to Private Chat App

This documentation provides a comprehensive guide to understanding and using the Private Chat App - a real-time secure chat application.


## Quick Links

<Cards>
  <Card href="/docs/getting-started" title="Getting Started" description="Setup instructions for local development" />
  <Card href="/docs/folder-structure" title="Folder Structure" description="Guide to the codebase organization" />
  <Card href="/docs/key-concepts" title="Key Concepts" description="Advanced patterns and how the app works" />
</Cards>


## Overview

Private Chat App is a real-time chat application built with modern web technologies including:

- **Next.js** - React framework for production
- **Elysia** - Fast and friendly Bun web framework
- **Upstash Redis** - Serverless Redis for real-time messaging
- **Fumadocs** - Documentation framework

## Getting Started

Explore the documentation to learn more about the different components and how they work together.
