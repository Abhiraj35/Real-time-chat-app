  ...
  <footer className="flex flex-col gap-6 px-4 pt-12 pb-3 md:px-8 md:pt-16 md:pb-5 border-b border-(--border)">
    <div className="grid grid-cols-1 gap-8 md:grid-cols-[minmax(0,1.1fr)_minmax(0,1.4fr)] md:items-start">
      <div className="flex flex-col gap-3">
        <Link href="/" className={`${pixelTriangleTheme.className} text-2xl font-bold tracking-[-0.03em] text-foreground`}>
          Flux
        </Link>
        <p className={`${pixelTriangleTheme.className} max-w-lg leading-[1.65] text-muted-foreground`}>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        {FOOTER_LINK_GROUPS.map((group) => (
          <div key={group.title} className={`${pixelTriangleTheme.className} flex flex-col gap-3`}>
